...
#endif
{
    ...
    font_->SetScale( 0.5 );
	healthText.reset(new hgeFont("font1.fnt"));
...
void Ship::Update(float timedelta)
{
	...
	{
		if (timerForRespawn < 5.f)
		{
			timerForRespawn += timedelta;
		}
		else
		{
			health = 100;
...
void Ship::Render()
{
	if (active)
	{
		sprite_->RenderEx(x_, y_, w_,2,2);
		std::string tempstr = "Health: ";
		tempstr.append(std::to_string(health));
		font_->printf(x_ + 5, y_ + 5, HGETEXT_LEFT, "%s", mytext_.c_str());
		healthText->printf(x_ - 5, y_ - 15, HGETEXT_LEFT, "%s", tempstr.c_str());
	}
}
